...
public class FacadeFragment extends QualityFragment {

    private static String TAG = "FacadeFragment";
    ...
    public void openPhoto(View view) {
        ...
        switch (view.getId()) {
            case R.id.pic01:
                if(quaSp.getBooleanFromXml(SharedManager.FACADE_PIC1)) {
                ...
                break;
            case R.id.pic02:
                if(quaSp.getBooleanFromXml(SharedManager.FACADE_PIC2)) {
                ...
                break;
            case R.id.pic03:
                if(quaSp.getBooleanFromXml(SharedManager.FACADE_PIC3)) {
                ...
                break;
            case R.id.pic04:
                if(quaSp.getBooleanFromXml(SharedManager.FACADE_PIC4)) {
    ...
    protected void putIvConfig2Xml(int position) {
        switch (position) {
            case QualityFragment.CAPTURE_REQUEST_CODE1:
                quaSp.putBooleanToXml(SharedManager.FACADE_PIC1, true);
                break;
            case QualityFragment.CAPTURE_REQUEST_CODE2:
                quaSp.putBooleanToXml(SharedManager.FACADE_PIC2, true);
                break;
            case QualityFragment.CAPTURE_REQUEST_CODE3:
                quaSp.putBooleanToXml(SharedManager.FACADE_PIC3, true);
                break;
            case QualityFragment.CAPTURE_REQUEST_CODE4:
                quaSp.putBooleanToXml(SharedManager.FACADE_PIC4, true);
                break;
            default:
                break;
        }
    ...
    private void initListener() {
        resultSp.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            ...
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                if(position == 0) {
                    quaSp.putStringToXml(SharedManager.FACADE_RESULT, "合格");
                }else if(position == 1) {
                    quaSp.putStringToXml(SharedManager.FACADE_RESULT, "不合格");
                }
            }
            @Override
        ...
        explainEdt.addTextChangedListener(new TextWatcher() {
            ...
            public void afterTextChanged(Editable s) {
                ...
                quaSp.putStringToXml(SharedManager.FACADE_REMARK, remark);
            }
        });
    }
    ...
    private void initData() {
        //初始化检测结果
        String result = quaSp.getStringFromXml(SharedManager.FACADE_RESULT, "").trim();
        switch (result) {
            case "不合格":
        ...
        //初始化说明信息
        String remark = quaSp.getStringFromXml(SharedManager.FACADE_REMARK, "").trim();
        explainEdt.setText(remark);
        //初始化图片显示
        boolean pic1 = quaSp.getBooleanFromXml(SharedManager.FACADE_PIC1);
        if(pic1) {
        ...
        boolean pic2 = quaSp.getBooleanFromXml(SharedManager.FACADE_PIC2);
        if(pic2) {
        ...
        boolean pic3 = quaSp.getBooleanFromXml(SharedManager.FACADE_PIC3);
        if(pic3) {
        ...
        boolean pic4 = quaSp.getBooleanFromXml(SharedManager.FACADE_PIC4);
        if(pic4) {
